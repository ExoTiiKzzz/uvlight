...
$categories = $oCategorie->db_get_all_without_filter();
...
$stmt = $conn->prepare("SELECT COUNT(*) AS allcount FROM ".DB_TABLE_CATEGORIE." WHERE cat_is_scat = 1 AND cat_is_visible = 1");
$stmt->execute();
$records = $stmt->fetch();
$totalRecords = $records['allcount'];

## Total number of records with filtering
$stmt = $conn->prepare("SELECT COUNT(*) AS allcount FROM ".DB_TABLE_CATEGORIE." WHERE cat_is_scat = 1 AND cat_is_visible = 1 ".$searchQuery);
$stmt->execute($searchArray);
$records = $stmt->fetch();
$totalRecordwithFilter = $records['allcount'];

## Fetch records
$stmt = $conn->prepare("SELECT cat_ID, cat_nom, cat_description, cat_ref 
                              FROM ".DB_TABLE_CATEGORIE." 
                              WHERE cat_is_scat = 1 ".$searchQuery."  AND cat_is_visible = 1
...
foreach($empRecords as $row){
    ...
    $data[] = array(
      ...
      "cat_ref" => $categories[$row["cat_ref"]]["cat_nom"],
      "actions"=>$actionrow
   );
}
