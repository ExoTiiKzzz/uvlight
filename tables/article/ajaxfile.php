...
$searchQuery = " ";
if($searchValue != ''){
   $searchQuery = " AND (article.art_nom LIKE :art_nom OR article.art_commentaire LIKE :art_commentaire OR article.fk_cat_ID IN (SELECT cat_ID FROM ".DB_TABLE_CATEGORIE." WHERE cat_nom LIKE :categorie) 
   OR article.fk_cas_ID IN (SELECT cas_ID FROM ".DB_TABLE_CASIER." WHERE cas_lib LIKE :casier))";
   $searchArray = array( 
        'art_nom'=>"%$searchValue%",
...
WHERE art_is_visible = 1 AND stockfinal.art_ID != 0 GROUP BY stockfinal.art_ID");
$stmt->execute();
$records = $stmt->fetchAll(PDO::FETCH_ASSOC);
$totalRecords = count($records);

## Total number of records with filtering
$stmt = $conn->prepare("SELECT COUNT(*) AS allcount, stockfinal.art_ID, stockfinal.art_commentaire, stockfinal.art_nom, stockfinal.fk_cat_ID, stockfinal.fk_cas_ID, stockfinal.stock 
FROM 
(SELECT (achetee.quantite_achetee - vendue.quantite_vendue) as stock, achetee.quantite_achetee, vendue.quantite_vendue, art_ID, art_commentaire, art_nom, fk_cat_ID, fk_cas_ID
   FROM article, 
   (SELECT SUM(Lign_quantite) AS quantite_vendue, fk_art_ID FROM lignes_commande WHERE Lign_is_vente = 1 GROUP BY fk_art_ID ) AS vendue, 
...
WHERE art_is_visible = 1 AND stockfinal.art_ID != 0 ".$searchQuery." GROUP BY stockfinal.art_ID");
...
(SELECT (achetee.quantite_achetee - vendue.quantite_vendue) as stock, achetee.quantite_achetee, vendue.quantite_vendue, art_ID, art_commentaire, art_nom, fk_cat_ID, fk_cas_ID
   ...
   WHERE art_is_visible = 1 AND  vendue.fk_art_ID = article.art_ID AND achetee.fk_art_ID = article.art_ID GROUP BY art_ID) as stockfinal,
   article
